      ...
      backups = {
        user-data = {
          location = {
            ...
            repositories = [
              {
                "path" = "ssh://${config.backup-user-prefix}-main-nix-1@${config.main-nix-1-private-ip}:${toString config.ports.exposed.ssh}/home/${config.backup-user-prefix}-main-nix-1/${config.data-dir}/backups/${hostName}";
                "label" = "remote-1";
              }
              {
                "path" = "ssh://${config.backup-user-prefix}-main-nix-2@${config.main-nix-2-private-ip}:${toString config.ports.exposed.ssh}/home/${config.backup-user-prefix}-main-nix-2/${config.data-dir}/backups/${hostName}";
                "label" = "remote-2";
              }
            ];
          };
