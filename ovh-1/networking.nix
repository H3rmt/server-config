{ lib, config, ... }: {
  systemd.network = {
    ...
    networks."10-eth" = {
      ...
      address = [
        "${config.server."${config.hostnames.ovh-1}".public-ip}/32"
        "${config.server."${config.hostnames.ovh-1}".public-ip-v6}/128"
      ];
      routes = [
    ...
    networks."30-wg" = {
      ...
      address = [
        "${config.server."${config.hostnames.ovh-1}".private-ip}/24"
      ];
